...
![fireflies](assets/banner.png)

Two algorithms drive it:

- **Flight** — steering behaviours (Reynolds' boids): a restless wander with a
  light touch of flocking.
- **Blinking** — pulse-coupled oscillators (Mirollo & Strogatz) plus slow
  frequency entrainment (Kuramoto), so the swarm drifts into sync on its own.
